...
int HW_ESP32Cam::colorDetect(void)
{
  ...
  if(num == 4)
  {
    if(color_info[2][2] > 0) //若w值大于0，则识别到颜色1
    {
      return 3;
    }
  }
...
uint8_t HW_ESP32Cam::readAILabel()
{
  uint8_t data[4] = {0};
  int num = WireReadDataArray(0x10, data, 4);
  if (num == 4) return data[0];
  return 0;
}

uint8_t HW_ESP32Cam::readAIShape()
{
  uint8_t data[4] = {0};
  int num = WireReadDataArray(0x20, data, 4);
  if (num == 4) return data[0];
  return 0;
}
