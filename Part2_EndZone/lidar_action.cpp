...
#define ACT_SLIDE_LEFT    11
#define ACT_SLIDE_RIGHT   12

// 换算参数（实测调整）
#define DEG_PER_ACT     15
#define MM_PER_ACT      1     // forwardMm = 执行次数
...
void action_slide(int dir) {
    if (!g_ctrl) return;
    int g = (dir > 0) ? ACT_SLIDE_LEFT : ACT_SLIDE_RIGHT;
    Serial.printf("  → 舵机: %s × 1\n", dir > 0 ? "左侧滑" : "右侧滑");
    g_ctrl->runActionGroup(g, 1);
    action_wait(800);
}

// dir: 1=左滑 -1=右滑, count=0表示连续重复
void action_slide_start(int dir) {
    if (!g_ctrl) return;
    int g = (dir > 0) ? ACT_SLIDE_LEFT : ACT_SLIDE_RIGHT;
    Serial.printf("  → 舵机: %s 连续\n", dir > 0 ? "左侧滑" : "右侧滑");
    g_ctrl->runActionGroup(g, 0);
}
...
void action_execute(const NavCmd& cmd) {
    ...
    if (cmd.turnAngle != 0) {
        ...
        if (cmd.smallTurn) {
            ...
            Serial.printf("  → 舵机: 小%s × 2\n",
                          cmd.turnAngle > 0 ? "左转" : "右转");
            g_ctrl->runActionGroup(g, 2);
            action_wait(TURN_WAIT_MS * 2);
        } else {
